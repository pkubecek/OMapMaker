...
def load_dmr_grid(dmr_path, resolution_points=1500j):
    ...
    if np.isnan(dmr_grid_interpolated).all():
        print("Fallback (DMR): 'Cubic' interpolace selhala, zkouším 'linear'.")
        dmr_grid_interpolated = griddata(points, z, (grid_x, grid_y), method='linear')
    
        if np.isnan(dmr_grid_interpolated).all():
            print("Fallback (DMR): 'Linear' interpolace selhala, používám 'nearest'.")
            dmr_grid_interpolated = griddata(points, z, (grid_x, grid_y), method='nearest')

    print("  -> Aplikuji dodatečné vyhlazení na kubickou mřížku (Gaussian filter)...")
    ...
    sigma_smooth = 1.4 #Míra zhlazení gridu
    dmr_grid = gaussian_filter(dmr_grid_filled, sigma=sigma_smooth)
...
def plot_lines(ax, grid_x, grid_y, data_grid, levels, style_id, zorder=15, smooth_factor=1.6):
    ...
    try:
        ...
        if style_props.get('color'):
            lc_props['colors'] = style_props.get('color')
        elif style_props.get('edgecolor'):
            lc_props['colors'] = style_props.get('edgecolor')
        if style_props.get('linewidth') is not None:
            lc_props['linewidths'] = style_props.get('linewidth')

        custom_linestyle = None
        if 'linestyle' in style_props and isinstance(style_props['linestyle'], tuple):
            custom_linestyle = style_props['linestyle']
        elif style_props.get('linestyle'):
            lc_props['linestyles'] = style_props.get('linestyle')
        ...
        for collection in cs.collections:
            ...
            if not processed_segments: 
                collection.remove() # Odstraníme původní kolekci, i když je prázdná
                continue
...
def vectorize_rocks(grid_x, grid_y, dmr_grid, transform, slope_threshold_deg=57):
    ...
    try:
        ...
        print(f"  -> Filtruji malé polygony skal (menší než {min_area:.2f} m^2)...")
        ...
        gdf.geometry = gdf.geometry.buffer(0.6)
...
def chaikin_smooth(verts, iterations=1):
    if iterations < 1:
        return verts
    ...
    for _ in range(iterations):
        ...
        is_closed = np.array_equal(verts[0], verts[-1])
...
def smooth_geometry(geom, s, k):
    try:
        geom_type = geom.geom_type
        if geom_type == 'LineString':
            return smooth_line(geom, s, k)
        ...
        # --- Zpracování Polygonů ---
            ...
            if exterior is None or exterior.is_empty:
                return geom
            smoothed_exterior = smooth_line(exterior, s, k)
            if smoothed_exterior.is_empty or not isinstance(smoothed_exterior, LineString):
                print("  -> Upozornění: Selhalo vyhlazení vnější hranice polygonu. Vracím původní.")
                return geom 
            smoothed_interiors = []
            for interior in geom.interiors:
                smoothed_interior = smooth_line(interior, s, k)
                if not smoothed_interior.is_empty and isinstance(smoothed_interior, LineString):
                    smoothed_interiors.append(smoothed_interior)
            
            return Polygon(smoothed_exterior, smoothed_interiors)
            
        elif geom_type == 'MultiLineString':
            smoothed_lines = [smooth_geometry(line, s, k) for line in geom.geoms]
            return MultiLineString([line for line in smoothed_lines if not line.is_empty])
            
        elif geom_type == 'MultiPolygon':
            smoothed_polygons = [smooth_geometry(poly, s, k) for poly in geom.geoms]
            return MultiPolygon([poly for poly in smoothed_polygons if not poly.is_empty])
            
        elif geom_type == 'GeometryCollection':
            smoothed_geoms = [smooth_geometry(g, s, k) for g in geom.geoms]
            return geom.__class__([g for g in smoothed_geoms if not g.is_empty])

        else:
            return geom
    except Exception as e:
        print(f"  -> Upozornění: Selhalo vyhlazování pro {geom.geom_type}: {e}. Vracím původní geometrii.")
        return geom
...
def add_vector_layers(ax, gdf, extent):        
    ...
    gardens = gdf[get_col(gdf, "landuse").isin([
        "residential", "allotments", "brownfield", "military", "commercial", 
        "construction", "industrial", "retail", "education", 
        "paddy", "animal_keeping", "highway", "cemetery", "landfill", "quarry"
    ])].copy()    
    boundaries = gdf[get_col(gdf, "boundary") == 'yes'].copy()
    cultivated_land = gdf[get_col(gdf, "landuse").isin(['cultivated_land', 'farmyards'])].copy()
    orchards_vineyards = gdf[get_col(gdf, "landuse").isin(["orchard", "vineyard"])].copy()
    power_lines = gdf[get_col(gdf, "power").isin(["line", "minor_line"])].copy()
    parking_areas = gdf[get_col(gdf, "amenity") == "parking"].copy()
    power_towers = gdf[get_col(gdf, "power") == "tower"].copy()

    # Styly
    style_major_road = SYMBOL_LIBRARY.get('osm_road_major', {}).get('props', {})
    style_road_secondary= SYMBOL_LIBRARY.get('osm_road_secondary', {}).get('props', {})
    ...
    style_orchard = SYMBOL_LIBRARY.get('osm_orchard_vineyard', {}).get('props', {})
    style_cultivated = SYMBOL_LIBRARY.get('cultivated_land', {}).get('props', {})
    style_power_line = SYMBOL_LIBRARY.get('power_line', {}).get('props', {})
    style_parking = SYMBOL_LIBRARY.get('osm_road_major', {}).get('props', {})
    ...
    if not orchards_vineyards.empty:
        plot_dotted_hatch(ax, orchards_vineyards, style_orchard, zorder=1.3)
    if not power_lines.empty:
        power_lines.plot(ax=ax, zorder=12, **style_power_line)
    if not parking_areas.empty:
        parking_areas.plot(ax=ax, zorder=5.1, **style_parking)
    if not power_towers.empty:
        ...
        if not symbol_info or symbol_info['type'] != 'point':
            ...
            symbol_path = symbol_info['path']
            symbol_props = symbol_info['props'].copy()
            symbol_props.setdefault('zorder', 12)
...
def map_zabaged_to_osm(gdf, filename):    
    ...
    if 'WaterCourse' in filename:
        ...
        print("  -> Detekován 'Lake', mapuji na 'natural' = 'water_areas'")
        gdf['natural'] = 'water'
    ...
    try:
        ...
        try:
            ...
            tags = {
                ...
                "power": ["line", "minor_line", "tower"],                
                "landuse": ["residential", "allotments", "brownfield", "military", "forest", "meadow", 
                            "commercial", "construction", "industrial", "retail", "education", 
                            "paddy", "animal_keeping", "highway", "cemetery", "orchard", "vineyard",
                            "landfill", "quarry"],
        ...
        if zabaged_paths: 
            ...
            zabaged_replaces = {
                ...
                'landuse_cultivated': False, 'amenity_parking': False, 'power_tower': False,
                'landuse_orchard_vineyard': False
            }
            ...
            for path in zabaged_paths:
                try:
                    ...
                    if 'landuse' in gdf.columns:
                         if gdf['landuse'].isin(["residential", "allotments", "brownfield", "military", "commercial", "construction", "industrial", "retail", "education", "paddy", "animal_keeping", "highway", "cemetery", "landfill", "quarry"]).any(): zabaged_replaces['landuse_residential_etc'] = True
                         if (gdf['landuse'] == 'meadow').any(): zabaged_replaces['landuse_meadow'] = True
                         if (gdf['landuse'] == 'cultivated_land').any(): zabaged_replaces['landuse_cultivated'] = True
                         if (gdf['landuse'] == 'forest').any(): zabaged_replaces['forest_wood'] = True
                         if gdf['landuse'].isin(["orchard", "vineyard"]).any(): zabaged_replaces['landuse_orchard_vineyard'] = True
                    all_gdfs.append(gdf)
                    if 'amenity' in gdf.columns:
                         if (gdf['amenity'] == 'parking').any(): zabaged_replaces['amenity_parking'] = True

                    if 'power' in gdf.columns:
                         if (gdf['power'] == 'tower').any(): zabaged_replaces['power_tower'] = True
                except Exception as e:
                ...
                if gdf_osm is not None and not gdf_osm.empty:
                    ...
                    if 'natural' in gdf_osm.columns:
                        if zabaged_replaces['natural_water']:
                            osm_mask_keep[(get_col(gdf_osm, 'natural') == 'water')] = False
                    ...
                    if 'landuse' in gdf_osm.columns:
                        if zabaged_replaces['landuse_residential_etc']:
                            osm_mask_keep[gdf_osm['landuse'].isin(["residential", "allotments", "brownfield", "military", "commercial", "construction", "industrial", "retail", "education", "paddy", "animal_keeping", "highway", "cemetery", "landfill", "quarry"])] = False
                        if zabaged_replaces['landuse_meadow']:
                            osm_mask_keep[(gdf_osm['landuse'] == 'meadow')] = False
                        if zabaged_replaces['landuse_cultivated']:
                            osm_mask_keep[gdf_osm['landuse'].isin(['cultivated_land', 'farmyards'])] = False
                        if zabaged_replaces['forest_wood']:
                            osm_mask_keep[(gdf_osm['landuse'] == 'forest')] = False
                        if zabaged_replaces['landuse_orchard_vineyard']:
                            osm_mask_keep[gdf_osm['landuse'].isin(["orchard", "vineyard"])] = False
        ...
        for i in range(tile_divisions):
            for j in range(tile_divisions):
                ...
                print(f"Ukládám dlaždici: {temp_file_path} (DPI=500)")
                plt.savefig(temp_file_path, dpi=500, bbox_inches='tight', pad_inches=0)
